...
class TextString extends React__default.Component {
    constructor() {
        super(...arguments);
    ...
    render() {
        ...
        return (React__default.createElement("span", { "data-slate-string": true, ref: this.ref, key: this.forceUpdateFlag ? 'A' : 'B' },
            text,
            isTrailing ? '\n' : null));
    }
}
...
const Editable = (props) => {
    ...
    const onDOMBeforeInput = React.useCallback((event) => {
        if (!readOnly &&
            ...
            if (type === 'insertText' &&
                ...
                const anchorNode = slate.Node.get(editor, selection.anchor.path);
                if (slate.Text.isText(anchorNode) && anchorNode.text.endsWith('\n')) {
                    native = false;
                }
                // and because of the selection moving in `insertText` (create-editor.tx).
                const { anchor } = selection;
                const inline = slate.Editor.above(editor, {
                    at: anchor,
                    match: n => slate.Editor.isInline(editor, n),
                    mode: 'highest',
                });
                if (inline) {
            ...
            switch (type) {
                ...
                case 'insertText': {
                    if (data instanceof DataTransfer) {
                        ...
                        if (native) {
                            asNative(editor, () => slate.Editor.insertText(editor, data));
                        }
                        else {
    ...
    return (React__default.createElement(ReadOnlyContext.Provider, { value: readOnly },
        ...
        , Object.assign({ "data-gramm": false, role: readOnly ? undefined : 'textbox' }, attributes, { 
            ...
            }, [readOnly]), onInput: React.useCallback((event) => {
